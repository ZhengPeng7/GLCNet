...
class MultiPartSpliter(nn.Module):
    def __init__(self, out_channels=None):
        ...
        if config.mps_blk == 'BasicDecBlk':
            ...
            num_blk = 4
            block_1 = nn.Sequential(*[BasicDecBlk(in_channels=1024 if idx_blk else inter_channels, out_channels=inter_channels if out_channels else out_channel_mps_blk) for idx_blk in range(num_blk)])
            block_2 = nn.Sequential(*[BasicDecBlk(in_channels=1024 if idx_blk else inter_channels, out_channels=inter_channels if out_channels else out_channel_mps_blk) for idx_blk in range(num_blk)])
            block_3 = nn.Sequential(*[BasicDecBlk(in_channels=1024 if idx_blk else inter_channels, out_channels=inter_channels if out_channels else out_channel_mps_blk) for idx_blk in range(num_blk)])
